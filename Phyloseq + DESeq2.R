...
# Family
# Calculate means, then re-normalize so each bar sums to 1 (100%)
...
# Filter strong changes (optional but nice visually)
...
# 1. Take the data frame used for your plot
Family_Percentages <- FamilyRel_collapsed %>%
  # Convert the 0-1 values to 0-100 percentages for the spreadsheet
  mutate(Percentage = Mean_Abundance * 100) %>%
  # Remove the raw fraction column to keep it clean
...
# 3. View the table in your console to verify
...
# Extract results (Severe vs Mild)
...
# Filter stronger effects (adjust if too few taxa)
sigtabgen <- subset(sigtab_annot, log2FoldChange > 1 | log2FoldChange < -1)
